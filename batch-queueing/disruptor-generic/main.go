...
func NewRingBuffer[T any](bufferSize int64) *RingBuffer {
	...
	rb := &RingBuffer{
		...
		elementSize: uintptr(reflect.TypeFor[T]().Size()),
	}
	...
	// Pre-touch pages
	for i := range buffer {
		buffer[i] = reflect.New(reflect.TypeFor[T]()).Elem().Interface().(T)
	}
	rb.buffer = unsafe.Pointer(&buffer[0])
...
func runTest(name string, bufferSize int64, waitStrategy WaitStrategy, consumerCount int) {
	...
	ringBuffer := NewRingBuffer[Msg](bufferSize)
